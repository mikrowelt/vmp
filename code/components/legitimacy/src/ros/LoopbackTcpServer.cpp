...
// the SC helper subprocess bridge (ros:subprocess, CEF) is not part of this
// build; the fake-success CreateProcessW path above is sufficient for the
// game to proceed, so the forwarded command line is simply logged.
void SubprocessPipe(const std::wstring& s)
{
	trace("SubprocessPipe: ignoring SC helper launch: %s\n", ToNarrow(s).c_str());
}
